...
def main():
    ...
    vec_env = FrankaGenesisVecEnvMultiInput(num_envs=1000, env_cfg=env_cfg, obs_cfg=obs_cfg, place_only=True, reward_cfg=reward_cfg, command_cfg=command_cfg, show_viewer=False)
    ...
    model = DDPG(
        ...
        vec_env,
        verbose=1,
        tensorboard_log="./ppo_franka_tensorboard/"
    )
    ...
    # Save the model
    model.save("ddpg_franka_vec")
    eval_env = FrankaGenesisVecEnvMultiInput(num_envs=1, env_cfg=env_cfg, obs_cfg=obs_cfg, reward_cfg=reward_cfg, command_cfg=command_cfg, place_only=True, show_viewer=True)
